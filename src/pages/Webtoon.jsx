...
export default function Webtoon() {
  ...
  const navItems = ["Details", "Episodes", "Comments"];
  ...
  const handleLikeClick = () => {
    ...
    if (!isLiked) {
      ...
      setLikeCount(likeCount - 1);
      webtoonData.likes -= 1;
      let newLikesArray = webtoonData?.likesArray.filter(
        (id) => id !== user._id
      );
  ...
  const confirmDelete = async () => {
    ...
    try {
      let res = await axios.delete(
        `${serverUrl}/twp/webtoon/${webtoonData._id}`,
        { withCredentials: true }
      );
      if (res.data) {
        navigate(user.isAuthor === true ? "/mywebtoons" : "/webtoons");
      }
    } catch (error) {
  ...
  if (!webtoonData) {
    return (
      <div className="flex justify-center items-center h-screen text-red-500">
        Failed to load webtoon. Please try again.
      </div>
    );
  }
    ...
    <div>
      <section className="w-[100%] h-[100%] max-w-[1000px] mx-auto px-[15px]">
        <div className="toon-image-wrapper w-full h-[350px] relative overflow-hidden rounded-lg shadow-md">
          ...
          <div
            className={`like-button-wrapper absolute bottom-4 z-10 flex items-center gap-2 cursor-pointer p-2 rounded-lg bg-black/50 hover:bg-black/70 transition-colors ${
              isEditable ? "right-16" : "right-4"
            }`}
          ...
          >
            <FontAwesomeIcon
              ...
              className={`text-2xl ${isLiked ? "text-red-500" : "text-white"}`}
            />
                ...
                <div className="fixed inset-0 bg-[#ffffff4f] bg-opacity-50 flex items-center justify-center z-50 px-[20px]">
                  <div className="bg-white rounded-lg p-8 max-w-md w-full">
                    ...
                    <p className="text-gray-700 mb-6">
                      Are you sure you want to delete this webtoon? This action
                      cannot be reversed.
                    </p>
        ...
        {showEpisodeUploadForm && (
          <div className="fixed overflow-scroll pt-[10px] inset-0 bg-[#ffffff4f] bg-opacity-50 flex items-center justify-center z-50 px-[20px]">
            <EpisodeUpload
              webtoonID={webtoonData._id}
              setShowEpisodeUploadForm={setShowEpisodeUploadForm}
            />
          </div>
        )}
      </section>
