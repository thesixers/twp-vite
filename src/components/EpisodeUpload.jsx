  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "https://twp2.onrender.com/twp/webtoon/episode",
        data,
  ...
  return (
    <Fragment>
      <div className="mx-auto relative top-[100px] w-full max-w-[500px] bg-white rounded-3xl shadow-md p-6 space-y-6">
        <X
          className="cursor-pointer absolute top-5 right-5 text-gray-500"
          size={25}
          onClick={() => !isLoading && setShowEpisodeUploadForm(false)}
        />
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div
              className={`border-2 border-dashed border-blue-200 rounded-md p-4 text-center ${
      ...
      {uploadStatus.message && (
        <div className="fixed inset-0 bg-[#ffffff5d] bg-opacity-60 flex items-center justify-center z-50 p-4">
          <div
            className={`bg-white p-6 sm:p-8 rounded-lg shadow-xl text-center w-full max-w-md mx-auto`}
          >
            <h3
