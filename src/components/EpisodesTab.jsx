...
export default function EpisodesTab({ webtoon, setWebtoonData, isEditable }) {
  const [showEpDeleteConfirmation, setEpShowDeleteConfirmation] =
  ...
  const confirmEpDelete = async () => {
    if (currentEpisode) {
      let res = await axios.delete(
        `${serverUrl}/twp/webtoon/episode/${currentEpisode}`,
        { withCredentials: true }
      );

      if (res.data) {
        let newEpisodes = webtoon.episodes.filter(
          (ep) => ep._id !== currentEpisode
        );
  ...
  if (!webtoon || !webtoon.chapters || webtoon.chapters.length === 0) {
    return (
  ...
  return (
    <div className="episodes animate-fadeIn">
      <div className="text-gray-600 episodes-wrapper flex flex-col gap-4 h-[400px] overflow-y-scroll">
        {webtoon.chapters.map((episode, index) => {
          return (
            <div
              className="ep flex justify-between items-center p-2 border-b border-gray-200 hover:bg-gray-50 transition-colors"
              key={episode._id || index}
            >
              <div className="left flex items-center gap-3">
                <Link
                  state={{ currentEpisode: episode, webtoonData: webtoon }}
                  to={`/read`}
                  className="img-link"
                >
